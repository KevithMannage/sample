...
export const unsubscribeFromDiscussion = async (req, res) => {
  try {
    const { user_id, discussion_id } = req.body;
    ...
    // Find the user's subscribed discussions
    const subscription = await SubscribedDiscussions.findOne({ user_id: userIdAsObjectId });

    if (!subscription) {
      return res.status(404).json({ message: 'No subscriptions found for this user.' });
    }

    // Remove the discussion from the user's subscriptions
    subscription.discussion_ids = subscription.discussion_ids.filter(
      (item) => item.discussion_id.toString() !== discussion_id
    );
