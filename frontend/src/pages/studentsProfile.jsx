...
const ProfessionalProfile = () => {
  ...
  const [relatedDiscussionLoading, setRelatedDiscussionLoading] = useState(false);
  const backendUrl = 'https://devthonbackend-production.up.railway.app';
  const [isEditing, setIsEditing] = useState(false);
  const [editFormData, setEditFormData] = useState({
    userName: '',
  ...
  useEffect(() => {
    const fetchProfileData = async () => {
      ...
      try {
        console.log('Fetching profile for userId:', userId);
        const response = await axios.get(`${backendUrl}/profile/grtprofile?userId=${userId}`);
        console.log('Profile response:', response.data);
  ...
  const fetchUserDiscussions = async () => {
    if (!username) {
      setDiscussions([]); // Show empty state instead of error
      return;
    }
  ...
  const fetchUserPosts = async () => {
    if (!username) {
      setPosts([]); // Show empty state instead of error
      return;
    }
  ...
  const fetchRelatedDiscussions = async () => {
    ...
    try {
      ...
      console.log('Related discussions response:', response.data);
      setRelatedDiscussions(response.data.discussions || []);
    } catch (err) {
      console.error('Error fetching related discussions:', err);
      setError(err.response?.data?.message || err.message || 'Failed to fetch related discussions');
    } finally {
      setRelatedDiscussionLoading(false);
    }
  };
  ...
  const handlesave = async () => {
    try {
      setLoading(true);
      const response = await axios.post(`${backendUrl}/user/updateuser`, { userId, ...editFormData });
      setProfileData(response.data.user);
      setIsEditing(false);
      setError(null);
    } catch (err) {
  ...
  const handleTabChange = (tab) => {
    ...
    if (tab === 'Questions') {
      ...
      fetchUserPosts();
    } else if (tab === 'Related Discussions') {
      ...
      {isEditing ? (
        ...
        <>
          ...
          <div className="profile-tabs">
            ...
            <button
              className={activeTab === 'Related Discussions' ? 'active' : ''}
              onClick={() => handleTabChange('Related Discussions')}
            >
              Related Discussions
            </button>
          </div>
          ...
          {activeTab === 'Questions' && (
            <div className="profile-content">
              ...
              {discussionLoading ? (
                ...
                <p>No discussions here. Start a new discussion to share your ideas.</p>
              )}
            </div>
          )}
          ...
          {activeTab === 'Answers' && (
            <div className="profile-content">
              ...
              {postLoading ? (
                ...
                <p>No posts here. Create a new post to share your insights.</p>
              )}
            </div>
          )}

          {activeTab === 'Related Discussions' && (
            <div className="profile-content">
              <h3>Related Discussions</h3>
              {relatedDiscussionLoading ? (
                <div>Loading related discussions...</div>
                ...
                <p>No related discussions found. Add interest areas to see relevant discussions.</p>
              )}
            </div>
          )}
        </>
